...
function handle (raptor, a0, a1) {
  if (
    ...
    if (a0.body) {
      raptor.connection().setSource(a0).handleObject(a0.body, function (err, res) {
        if (err) {
          raptor.emit(err)
        } else {
...
function attach (raptor, server) {
  ...
  if (server instanceof dgram.Socket) {
    server.on('message', function (msg, rinfo) {
      raptor.connection().setSource({ msg: msg, rinfo: rinfo }).handleBuffer(msg, function (err, res) {
        if (err) {
...
module.exports = function (raptor) {
  return {
    serve: function (type, port) { return serve(raptor, type, port) },
